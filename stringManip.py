...
def sentenceSplit(string):  # --- input to the function is a string
    ...
    for i in range(0, len(sentenceList) - 1):
        ...
        if len(
                sentence) >= MAX_CHARACTER_LENGTH_PER_LINE and ';' in sentence:  # --- long sentence which can be split by ';'
            stringDelim = '\;'
            ...
            for word in sentenceSplit:
                # print('\nword=', word)
                if len(sentence) + len(
                        word) > MAX_CHARACTER_LENGTH_PER_SPLIT_LINE:  # -- check if the next word would exceed the permitted length
                    finalList.append(sentence)
                    sentence = word + ' '
                else:
...
def paragraphSplit(sentenceList):  # --- function is called with a list of sentences
    ...
    mysentenceList = periodSplit(
        paragraph_text)  # --- call the function to split the paragraph into sentences by periods
    mysentenceList = generalSplit(
        mysentenceList)  # --- call the function to split the paragraph into sentences by periods
