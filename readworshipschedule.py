...
def readWS():
   ...
   #----- read the worship schedule file extracted from Discord into a list / array
    ...
    #print('\nSongs=', songs)
    np.savetxt(filelist.SongsFileName, songs, fmt='%s')      #--- use numpy module to write 2-dimensional list of songs to file
...
def parsesong(songs, line):
    ...
    songs.append([song_name, presentation_order])      #--- update list to hold the worship songs and presentation order for each
