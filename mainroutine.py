...
import stringsplit  # --- module to split a string by a number followed by a space
...
def call_passagelookup():
    ...
    scripture = passagelookup.get_esv_text(passages, numbers).replace('[', '').replace(']', '').replace('–',
                                                                                                        '-').replace(
        '\n\n  ', ':  ')
