...
#---------------- Verify the bulletin file exists ------------
    ...
    extractpdftext.fitz_extract_text(filelist.PDFBulletinFilename, filelist.TextPDFBulletinFilename)       #--- read the pdf bulletin and convert to text file
...
def parsebulletin():
    ...
    for line in Lines:
            ...
            if 'Doxology' in Lines[count+1]:
                ...
                for count in range(count, len(Lines) - 1):
                    if 'presbyterianchurch' in Lines[count].replace(" ", '').replace('\t', '').lower():
                        ...
                        prefix, text = Lines[count].split(' ', 1)          #--- split the line at the first space to remove the #'s
                ...
                textFile = open(filelist.AnnouncementFileName, 'w', encoding='utf-8',errors='ignore')  #--- output announcement text file
        ...
        if 'calltoworship' in line.replace(" ", '').replace('\t', '').lower():
            ...
            for count in range(count, len(Lines)-1):
                ...
                if 'songofapproach' in Lines[count+1].replace(" ", '').replace('\t', '').lower():
                    ...
                    textFile = open(filelist.CallToWorshipFileName, 'w', encoding='utf-8',errors='ignore')       #--- output call to worship text file
                    textFile.write(str(body_text))
                    textFile.close()
                    break
                else:
        ...
        if 'scripturereading' in Lines[count].replace(" ", '').replace('\t', '').lower():
            ...
            textFile = open(filelist.ScriptureFileName, 'w', encoding='utf-8',errors='ignore') #--- output call to worship text file
            textFile.write(str(body_text))
            textFile.close()
        ...
        if 'sermon' in Lines[count].replace(" ", '').replace('\t', '').lower():
            ...
            textFile = open(filelist.BulletinSermonFilename, 'w', encoding='utf-8',errors='ignore') #--- output call to worship text file
            textFile.write(str(body_text))
            textFile.close()
        ...
        if 'affirmationoffaith' in Lines[count].replace(" ", '').replace('\t', '').lower():
            ...
            aofextract(aof_tag, count, Lines)          #--- Call the aofextract routine to write the affirmation of faith to file
            continue
